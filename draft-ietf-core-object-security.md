...
If the verification of the response succeeds, and the received Partial IV was greater than the Notification Number, then the client SHALL update the corresponding Notification Number with the received Partial IV. The operation of validating the Partial IV and updating the replay protection data MUST be atomic. If further CoAP processing requires Observation values to be present and verification can be sure that the limit of 2^24 messages in 128 seconds is not exceeded, the client MAY use the three least significant bytes of the Partial IV as the Observe value when passing the unprotected message on to CoAP processing.
